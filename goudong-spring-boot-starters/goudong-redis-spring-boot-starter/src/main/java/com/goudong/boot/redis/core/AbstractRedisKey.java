 ...
 * redis key
 * <ul>
 *     <li>redisKey的公共属性和行为定义，之所以有了枚举{@link RedisKeyTemplateProviderEnum}过后，还编写该类的原因是，强制某些规则，使其子类必须、</li>
 *     <li>遵循某些规则，例如构造函数。这样在后期使用时，才会不会因为用户的错误操作导致一部分功能执行不成功。</li>
 *     <li>举个例子：在redisTool在解决缓存雪崩问题时，有些参数规则必须要存在。</li>
 *     <li>使用方式，只需要继承该类即可，然后使用, 例如{@link SimpleRedisKey}</li>
 *     <li>使用枚举比较好 {@link RedisKeyTemplateProviderEnum}</li>
 * </ul>
