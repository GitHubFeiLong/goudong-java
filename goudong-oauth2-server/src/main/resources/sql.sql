...
MySQL Backup
Database: goudong
Backup Time: 2021-05-08 16:30:28
*/
...
DROP TABLE IF EXISTS `goudong`.`flyway_schema_history`;
...
BEGIN;
LOCK TABLES `goudong`.`authority_ignore_resource` WRITE;
DELETE FROM `goudong`.`authority_ignore_resource`;
...
COMMIT;
BEGIN;
LOCK TABLES `goudong`.`authority_menu` WRITE;
DELETE FROM `goudong`.`authority_menu`;
INSERT INTO `goudong`.`authority_menu` (`uuid`,`url`,`method`,`menu_name`,`parent_uuid`,`remark`,`is_delete`,`update_time`,`create_time`) VALUES ('1', '/api/user/hello', 'get', NULL, NULL, NULL, 0, '2021-04-03 17:52:07', '2021-04-03 17:52:07');
UNLOCK TABLES;
COMMIT;
BEGIN;
LOCK TABLES `goudong`.`authority_role` WRITE;
DELETE FROM `goudong`.`authority_role`;
INSERT INTO `goudong`.`authority_role` (`uuid`,`role_name`,`role_name_CN`,`remark`,`is_delete`,`update_time`,`create_time`) VALUES ('1', 'ROLE_ADMIN', '管理员', NULL, 0, '2021-04-03 16:52:11', '2021-04-03 16:52:11');
UNLOCK TABLES;
COMMIT;
BEGIN;
LOCK TABLES `goudong`.`authority_role_menu` WRITE;
DELETE FROM `goudong`.`authority_role_menu`;
INSERT INTO `goudong`.`authority_role_menu` (`uuid`,`role_uuid`,`menu_uuid`,`is_delete`,`update_time`,`create_time`) VALUES ('1', '1', '1', 0, '2021-04-03 17:52:14', '2021-04-03 17:52:14');
UNLOCK TABLES;
COMMIT;
BEGIN;
LOCK TABLES `goudong`.`authority_user` WRITE;
DELETE FROM `goudong`.`authority_user`;
INSERT INTO `goudong`.`authority_user` (`uuid`,`username`,`password`,`email`,`phone`,`nickname`,`remark`,`valid_time`,`is_delete`,`update_time`,`create_time`,`qq_open_id`) VALUES ('1', 'admin', '$2a$10$8ptteV1xP51AjOS/u6NAle/1Pw2BUsS.D/mbGuhauf.qc.oTikeAy', '1', '1', NULL, NULL, '2021-05-08 15:51:25', 0, '2021-04-03 15:48:39', '2021-04-03 15:48:39', NULL);
UNLOCK TABLES;
COMMIT;
BEGIN;
LOCK TABLES `goudong`.`authority_user_role` WRITE;
DELETE FROM `goudong`.`authority_user_role`;
INSERT INTO `goudong`.`authority_user_role` (`uuid`,`user_uuid`,`role_uuid`,`is_delete`,`update_time`,`create_time`) VALUES ('1', '1', '1', 0, '2021-04-03 16:52:22', '2021-04-03 16:52:22');
UNLOCK TABLES;
COMMIT;
BEGIN;
LOCK TABLES `goudong`.`flyway_schema_history` WRITE;
DELETE FROM `goudong`.`flyway_schema_history`;
INSERT INTO `goudong`.`flyway_schema_history` (`installed_rank`,`version`,`description`,`type`,`script`,`checksum`,`installed_by`,`installed_on`,`execution_time`,`success`) VALUES (1, '1.0.0', 'init', 'SQL', 'V1.0.0__init.sql', 1985037391, 'root', '2021-04-02 21:20:20', 350, 1);
UNLOCK TABLES;
COMMIT;
